...
export function ChipPicker({ value, options, onChange, allowNone, noneLabel = 'Sem categoria', icon, title }) {
  const { open, setOpen, pos, triggerRef, popRef } = useAnchoredPopover()
  const current = options.find(o => o.id === value) || null
  return (
    <div className={styles.chipWrap}>
      <button ref={triggerRef} type="button" className={`${styles.chipPick} ${!current ? styles.chipPickNone : ''}`} onClick={() => setOpen(o => !o)} aria-haspopup="menu" aria-expanded={open} title={title}>
